...
+ (BOOL)tl_jr_swizzleMethod:(SEL)origSel_ withMethod:(SEL)altSel_ withClass:(Class)withClass error:(NSError**)error_ {
	...
	class_addMethod(self,
					altSel_,
					class_getMethodImplementation(withClass, altSel_),
					method_getTypeEncoding(altMethod));
	...
	void *iterator = NULL;
	struct objc_method_list *mlist = class_nextMethodList(origClass, &iterator);
	while (mlist) {
		int method_index = 0;
		for (; method_index < mlist->method_count; method_index++) {
			if (mlist->method_list[method_index].method_name == origSel_) {
	...
	if (!directOriginalMethod || !directAlternateMethod) {
		Method inheritedOriginalMethod = NULL, inheritedAlternateMethod = NULL;
		if (!directOriginalMethod) {
			inheritedOriginalMethod = class_getInstanceMethod(origClass, origSel_);
			if (!inheritedOriginalMethod) {
				SetNSError(error_, @"original method %@ not found for class %@", NSStringFromSelector(origSel_), [origClass className]);
				return NO;
			}
		}
		if (!directAlternateMethod) {
			inheritedAlternateMethod = class_getInstanceMethod(withClass, altSel_);
			if (!inheritedAlternateMethod) {
				SetNSError(error_, @"alternate method %@ not found for class %@", NSStringFromSelector(altSel_), [withClass className]);
				return NO;
			}
		}
		...
		class_addMethods(origClass, hoisted_method_list);
	}
...
+ (BOOL)tl_jr_swizzleClassMethod:(SEL)origSel_ withClassMethod:(SEL)altSel_ withClass:(Class)withClass error:(NSError**)error_ {
	return [GetClass((id)self) tl_jr_swizzleMethod:origSel_ withMethod:altSel_ withClass:withClass error:error_];
}
